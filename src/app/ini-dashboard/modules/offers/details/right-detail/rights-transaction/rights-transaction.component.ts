...
@Component({
  selector: 'app-rights-transaction',
  templateUrl: './rights-transaction.component.html',
  styleUrls: ['./rights-transaction.component.css']
})
export class RightsTransactionComponent implements OnInit {
  rightId: any;
  container: any = {};
  ...
  emptyState = {
    title: 'No Rights Transactions',
    subTitle: 'There are no rights transactions available yet',
  };
  ...
  ngOnInit() {
    this.rightId = this.aRoute.snapshot.paramMap.get('id');
    this.appContext
      .getUserInformation()
      .pipe(take(1))
      .subscribe({
  ...
  getTransactions() {
    ...
    .pipe(
      ...
      switchMap(() => {
        this.container['transactionsLoading'] = true;
      return  this.http
        .get(`${environment.baseApiUrl}/rights/txn?page=${this.paginator.pageIndex}&perPage=${
          this.paginator.pageSize}${this.rightId ? '&rightId='+this.rightId :''}`)
